...
export default function GoogleCalendarSync() {
  ...
  useEffect(() => {
    api.get('/google-calendar/status')
      ...
      .catch(() => setStatus({ conectado: false, pronto: false }))
      .finally(() => setLoading(false))
  }, [])

  async function handleConectar() {
    if (status && !status.pronto) {
      toast.error('Google Calendar indisponivel neste servidor.')
      return
    }
    ...
    try {
      ...
      toastErro(err, 'Nao foi possivel iniciar a conexao com o Google.')
    }
  }
  ...
  async function handleDesconectar() {
    if (!confirm('Desconectar o Google Calendar? Os eventos ja criados nao serao removidos.')) return
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-xs text-lexiona-400 leading-relaxed">
          A sincronizacao cria eventos com tema, objetivos e conteudos de cada aula.
        </p>

        <div className="flex flex-col sm:flex-row gap-3">
          <button
            onClick={handleSync}
            disabled={syncing || status?.pronto === false}
  ...
  const indisponivel = status?.pronto === false

  return (
    <div className="space-y-4">
      <div className={`flex items-start gap-3 rounded-xl px-4 py-4 border ${
        indisponivel ? 'bg-amber-50 border-amber-200' : 'bg-lexiona-50 border-lexiona-100'
      }`}>
        <AlertCircle size={18} className={`${indisponivel ? 'text-amber-600' : 'text-lexiona-400'} flex-shrink-0 mt-0.5`} />
        <div>
          <p className="text-sm font-semibold text-lexiona-800">
            {indisponivel ? 'Google Calendar indisponivel' : 'Google Calendar nao conectado'}
          </p>
          <p className="text-xs text-lexiona-500 mt-1 leading-relaxed">
            {indisponivel
              ? 'Configure as credenciais Google e instale as bibliotecas do backend antes de conectar uma conta.'
              : 'Conecte sua conta Google para sincronizar suas aulas com o Google Calendar.'}
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full bg-lexiona-600 hover:bg-lexiona-700 text-white font-semibold py-3 rounded-xl transition text-sm disabled:opacity-50 disabled:cursor-not-allowed"
      >
